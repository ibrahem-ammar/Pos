...
@section('title') @lang('site.add') @lang('site.product') @endsection
...
<form role="form" method="POST" action="{{ route('dashboard.products.store') }}" enctype="multipart/form-data">
            ...
            <div class="card-body">
                <div class="tab-content" id="custom-tabs-four-tabContent">
                    ...
                    <div class="tab-pane fade {{ $index==0? 'show active' : '' }}" id="{{ $local }}" role="tabpanel" aria-labelledby="{{ $local . '-tab' }}">
                        <input type="text" class="form-control mb-2" value="{{ old($local.'.name') }}" id="{{'name_' . $local}}" placeholder=" @lang('site.product') @lang('site.name')" name="{{$local}}[name]">
                        {{-- <input type="text" class="form-control mt-2" value="{{ old($local.'.description') }}" id="{{'description_' . $local}}" placeholder=" @lang('site.product') @lang('site.description')" name="{{$local}}[description]"> --}}
                        <textarea name="{{$local}}[description]" class="form-control mt-2 ckeditor" id="{{'name_' . $local}}" cols="30" rows="5" >{{ old($local.'.description') }}</textarea>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="form-group">
        ...
        <input type="number" name="purchase_price" value="{{ old('purchase_price') }}" class="form-control" placeholder="0">
    </div>

    <div class="form-group">
        <label for="sale_price">@lang('site.sale_price')</label>
        <input type="number" name="sale_price" value="{{ old('sale_price') }}" class="form-control" placeholder="0">
    </div>
